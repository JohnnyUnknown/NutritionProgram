...
class Program
{
    ...
    {
        SetWindowSize(92, 45);  // Установка рекомендованного размера консоли
        NutritionProgram np = new NutritionProgram();
    }
}
...
class Product
{
    protected internal string Name { get; set; }       // Название продукта
    protected internal ProductStruct prod;
    ...
    protected internal Product(string name)
    {
        Name = name.ToUpper();
        Name = Name.Replace(' ', '_');
        SetNutrientsProd();
    ...
    protected internal Product(string name, double protein, double fat, double carbohydrates)
    {
        Name = name.ToUpper();
        Name = Name.Replace(' ', '_');
    ...
    protected internal void RenameProd()
    {
...
class ProductsTable : Product
{
    protected SortedList<string, Product> products = new SortedList<string, Product>();
...
class DayNutrition : ProductsTable
{
    protected NutrientsStruct[] NutrientsPerDay;  // Потребление КБЖУ за день
    protected Product[] Breakfast;          // Потребление КБЖУ за завтрак
    protected Product[] Brunch;             // Потребление КБЖУ за второй завтрак
    protected Product[] Lunch;              // Потребление КБЖУ за обед
    protected Product[] AfternoonTea;       // Потребление КБЖУ за полдник
    protected Product[] Dinner;             // Потребление КБЖУ за ужин
    protected Product[] Supper;             // Потребление КБЖУ за поздний ужин
    private const int size = 20;            // Размер таблиц приемов пищи
    ...
    protected internal void AddProdToMeal(Product[] ns, double[] arr)
    {
        Clear();
        Write("Добавление продукта в рацион.\n\n");
